...
SUBROUTINE GAUSS(KPTS,PGAUSX,PGAUSH)
  ...
  DO J1=1,KPTS
     !
     PGAUSX(J1) = (1._rkind + PGAUSX(J1))*0.5_rkind
...
subroutine Gauss2DQuad(fxy,order,res)
  use prec_const
  real(rkind), external :: fxy
  integer, intent(in) :: order
  real(rkind), intent(out) :: res
  real(rkind), dimension(order+1) :: points,weights
  integer :: i,j,nmax
  ...
  call gauss(order,points,weights)

  res = 0._rkind
  do i=1,nmax
     do j=1,nmax
        res = res + fxy(points(i),points(j))*weights(i)*weights(j)
     end do
  end do

end subroutine Gauss2DQuad
